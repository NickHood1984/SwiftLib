...
enum ChineseMetadataConsensus {

    /// A metadata contribution from a single source.
    struct SourceContribution: Sendable {
        let source: MetadataSource
        let reference: Reference
        let titleScore: Double // similarity to seed title
        let priority: Int // lower = more authoritative
    }
