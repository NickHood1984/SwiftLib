...
public actor ParallelSourceFetcher {
    ...
    public func fetchByDOI(_ doi: String, forceRefresh: Bool = false) async -> FetchResult {
    ...
    public func fetchByTitle(_ title: String, forceRefresh: Bool = false) async -> FetchResult {
        // Use withTaskGroup instead of async let: async let inside an actor crashes
        // on swift_task_dealloc when the parent task is cancelled externally.
    ...
    public func fetchByIdentifier(_ identifier: MetadataFetcher.Identifier, forceRefresh: Bool = false) async -> FetchResult {
        switch identifier {
        case .doi(let doi):
            return await fetchByDOI(doi, forceRefresh: forceRefresh)
        ...
        case .pmid(let pmid):
            ...
            if let ref = try? await MetadataFetcher.fetchFromPMID(pmid, forceRefresh: forceRefresh) {
                ...
                if let doi = ref.doi, !doi.isEmpty {
                    let enrichResult = await fetchByDOI(doi, forceRefresh: forceRefresh)
        ...
        case .isbn(let isbn):
            var results: [SourceResult] = []
            if let ref = try? await MetadataFetcher.fetchFromISBN(isbn, forceRefresh: forceRefresh) {
                results.append(SourceResult(source: .crossRef, reference: ref))
            }
            return FetchResult(sources: results, discoveredDOI: nil)
        }
