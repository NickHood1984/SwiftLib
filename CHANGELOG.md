...
- 万方/维普候选改用真实多字段评分（题名 0.60 + 第一作者 0.25 + 年份 0.10 + 期刊 0.05，与 CNKI 同一量纲），移除原先 `max(titleScore, 0.45)` 的人为下限——弱相关结果不再以"45%"混入候选列表，也不再在合并排序中挤掉评分更诚实的 CNKI 候选。
- 万方/维普候选的中文作者名改走 Han 感知解析（与 CNKI 路径一致），修复带空格中文名被按西文规则拆成姓/名、污染 CSL 引文输出的问题；少数民族姓名（含 "·"）保持完整。
- 中文浏览器检索通道（万方/维普）在批量刷新并发占用时改为排队等待而非静默返回"无结果"——此前并发刷新 3 条中文文献时，只有第一条能用上回退通道，其余直接报"未找到候选"。

- 统一浮层视觉风格：新增 `slOverlaySurface` 设计令牌（统一背景、0.5pt 发丝描边、阴影、圆角），进度提示条、CSL 消息条、待确认队列通知三类浮层共用同一套外观；字体与间距统一改用 `SLDesign` 令牌。
- 待确认元数据窗口改用隐藏式标题栏：去掉窗口顶部与内容割裂的深色标题条带，标题只保留内容区一处，不再与系统标题栏重复。
- 精简候选卡片信息：删除与上方作者/期刊/摘要重复的"已有：…"字段罗列，"匹配: title / author"改为中文"匹配依据：题名 · 作者"，条目类型（如 Journal Article）改为中文显示（期刊论文、学位论文等）。
- 元数据刷新新增单源软超时（默认 15 秒）：CrossRef/OpenAlex/Semantic Scholar 并行抓取不再等待最慢的源（S2 429 重试链可拖 30–40 秒），超时即取消掉队源并用已到达的结果合并，最多损失部分富化字段，不影响主书目记录。
- 标题检索发现的 DOI 现在会经 CrossRef 反向验证：CrossRef 记录与查询标题明显不符时，丢弃该 DOI 并从各源结果中清除，防止错误 DOI 经字段合并覆盖正确的标题/作者；OpenAlex 与 S2 的 DOI 冲突时按标题匹配强度取舍。

### 修复

- 修复"正在刷新元数据…"永久挂起、结果不回传的问题：刷新管线的 90 秒总超时通过 `withTaskGroup` 竞速实现，而任务组退出前必须等待所有子任务结束——CNKI 与百度学术的人机验证等待（`requestVerification`）此前不响应任务取消，一旦触发验证（尤其是百度验证窗口从未在主界面接线呈现，等待完全不可见），刷新便永远无法返回。现验证等待改为 `withTaskCancellationHandler` 包装，取消即恢复并自动关闭验证窗口；同时补上百度学术验证面板在主界面的呈现链路。
- 进度提示条上的取消按钮现在对单条刷新同样生效（此前仅能取消批量刷新），取消后不会被迟到的旧结果覆盖提示。
- 离屏 HTML 解析（`HTMLLoadDelegate`）补充 15 秒加载超时，防止 WebKit 偶发不回调 `didFinish` 时卡死整条元数据管线。
- 重新接回 v1.4.0 重构中丢失的百度学术回退调用链：万方/维普均无候选时，会以百度学术聚合检索兜底（统一进入人工确认队列，不做自动验证），README 承诺的中文回退链路恢复完整。
- 修复 `ChineseMetadataConsensus` 作者选取的并列取舍缺陷：全部来源都没有中文作者名时，旧实现会把作者顺序交给优先级最低的源（如 OpenAlex 覆盖 CrossRef/CNKI）；现在仅在存在中文作者名时覆盖，并列时按来源优先级取舍。
- 修复导入 CSL 样式必然失败的问题：真实样式的 `<id>` 为 URL（如 `http://www.zotero.org/styles/apa`），此前直接以 id 为文件名落盘，路径分隔符导致写入抛错。现统一清洗为单一文件名（含稳定哈希防冲突），文件导入、Word 插件上传、XML 导入三条路径全部修复。
- 修复重新导入或删除 CSL 样式后渲染不更新的问题：导入/删除现在会同时失效 citeproc-js 引擎池与渲染字符串缓存，更新后的样式立即生效，无需重启。
- 大幅加快引用预览渲染：`CiteprocJSCoreEngine` 每次渲染不再重建整个 `CSL.Engine`（每次需重新解析样式与 locale XML，数百毫秒），改用 citeproc-js 的 `restoreProcessorState` 快速清空处理器状态，失败时才回退到完整重建。

- 修复 `HostRateLimiter` 在并发下完全失效的 actor 重入问题：原实现先 `Task.sleep` 再更新 `lastDispatchAt`，批量导入/批量刷新时所有并发请求读到同一过期时间戳后同时发出，导致 Semantic Scholar 等限速敏感源批量 429、触发熔断。现改为挂起前预占发送时隙，并发调用按间隔排队。
- 修复 `HostCircuitBreaker` 半开逻辑：熔断打开期间不再放行"立即探针"（冷却期内严格快速拒绝）；冷却到期后保留 open 标记，确保只放行一个探针，杜绝并发请求经 closed 路径全部漏过；新增探针超时接管，避免探针任务被取消后熔断器永久卡死。
- 为 `HostRateLimiter` 与 `HostCircuitBreaker` 补齐单元测试（此前为元数据管线中唯二零覆盖组件）。
